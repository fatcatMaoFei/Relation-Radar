...
CI / 测试管理约定（总体）：
- CI 分为 **core** / **optional** 两层：core 覆盖最小闭环（DB + ingest + RAG + 基础 API），optional 用于耗时或依赖重的检查（如 OCR、音频、远端 LLM、训练等），可放在单独 workflow 或按需启用。
- `scripts/` 下的脚本级测试优先作为阶段性验收工具使用，功能稳定后应考虑将关键逻辑迁移到 `tests/`（如使用 pytest），脚本可降级为开发工具或删除。
- 每个小版本（如 v0.1、v0.2）收尾前，建议做一次 CI & scripts 的清理：梳理哪些脚本/optional job 仍有价值，哪些可以合并、下线或移至 `scripts/legacy/` 等归档位置。
...
测试与 CI 约定（v0.1 总体）：
- 每个 PR 尽量配套一个「脚本级测试 / demo」（放在 `scripts/` 目录，例如 `test_*.py` 或演示脚本），用于本地快速验证核心路径。
- 随着能力完善，逐步把这些脚本挂到 CI（GitHub Actions）里运行，保证主干上的最小闭环始终可用。
...
- **验收标准**  
  ...
  - 建议配套一个脚本级测试（如 `scripts/test_rag.py` / `scripts/test_ingest.py`），并在 CI 中运行，保证端到端链路持续可用。
...
测试与 CI 约定（v0.2 总体）：
- 新增的输入通道（OCR / 音频）和 Web/API 功能，优先通过 `scripts/` 目录下的脚本或轻量测试用例做端到端验证。
- 对关键链路（文本抽取、ingest、提醒）的脚本级测试，逐步挂入 CI 或单独 workflow，保证“录入 → 分析 → 提醒”的闭环长期可用。
...
- **验收标准**  
  ...
  - 建议新增脚本级测试（如 `scripts/test_llm_local.py`），覆盖最小问答/抽取调用，并视本地硬件条件决定是否在 CI 中启用。
...
- **验收标准**  
  ...
  - 建议提供脚本（如 `scripts/test_extract_events_qwen.py`），验证多事件抽取与入库流程，可纳入 CI 或预发布流水线。
...
- **验收标准**  
  ...
  - 建议新增脚本（如 `scripts/test_ingest_ocr.py`），使用固定测试图片跑通 OCR → 抽取 → 入库链路，并在 CI 中以可选 job 运行（考虑 OCR 依赖）。
...
- **验收标准**  
  ...
  - 建议新增脚本（如 `scripts/test_ingest_audio.py`），对示例音频跑通语音 → 文本 → 抽取 → 入库链路，可在 CI 中按需启用。
...
- **验收标准**  
  ...
  - 建议提供简单请求级测试（如 `scripts/test_web_api.py` 或轻量 `pytest`），覆盖主要接口的 200/404 路径，并在 CI 中运行。
...
- **验收标准**  
  ...
  - 建议保留一个 Web 层的冒烟脚本（如 `scripts/smoke_web_timeline.py` 使用 `requests`/Playwright 等），在 CI 或预发布环境中确认页面可加载并返回 200。
...
- **验收标准**  
  ...
  - 建议为 `scripts/run_reminders.py` 增加可重复运行的测试模式（例如 `--test`），并在 CI 中用固定数据集跑一遍，验证生成提醒的数量和基本字段。
...
- **验收标准**  
  ...
  - 建议补充脚本或测试用例（如 `scripts/test_feedback_flow.py`），覆盖“问答 → 评分入库 → 从 DB 查询反馈”的完整路径，有条件时纳入 CI。
...
测试与 CI 约定（v0.3 总体）：
- MCP server、远端大模型和训练脚本依赖较重，推荐通过 `scripts/` 和/或 `tests/` 下的用例分层验证：基础单元测试 + 少量集成脚本。
- CI 中至少应保留“接口层”冒烟脚本（如 MCP 工具调用、训练数据构建），GPU 训练可放在单独 workflow 或手动触发的 pipeline 中。
...
- **验收标准**  
  ...
  - 建议提供 `scripts/test_mcp_tools.py`，分别调用 `search_events` / `get_person_summary` / `log_feedback`，在 CI 中作为 MCP 层的冒烟测试。
...
- **验收标准**  
  ...
  - 建议新增脚本（如 `scripts/test_remote_llm_chain.py`），在启用增强模式配置下跑一到两个问题，验证链路连通性；可在 CI 中按需或在特定环境变量存在时运行。
...
- **验收标准**  
  ...
  - 建议将 `build_training_dataset.py` 的核心逻辑拆分为可测试函数，并提供脚本/测试用例验证在样例 DB 上生成的 JSONL 结构与字段；训练脚本可在单独 CI workflow 中做最小步长的“干跑”或短轮次训练。
...
- **验收标准**  
  ...
  - 建议增加脚本（如 `scripts/test_lora_loading.py`），验证在加载/切换不同 LoRA 权重时模型能正常生成回答，可作为 GPU 环境下的可选 CI 步骤。
